...
class DNDBeyondSpellParser:
    """Parser for D&D Beyond spell data."""
    ...
    def parse_all_spells(self) -> SpellList:
        """Parse all spells from D&D Beyond JSON and return SpellList object."""
        return self._parse_dndbeyond_spelllist()
    ...
    def _parse_spell_components(self, components: List[int], components_description: str = "") -> SpellComponents:
    ...
    def _format_duration(self, duration_data: Dict[str, Any]) -> str:
        ...
        unit_str = self.DURATION_UNIT_MAP.get(unit, unit.lower() if unit else "instantaneous")
    ...
    def _format_casting_time(self, activation_data: Dict[str, Any]) -> str:
        ...
        time_str = self.ACTIVATION_TYPE_MAP.get(activation_type, "1 action")
    ...
    def _format_area(self, range_data: Dict[str, Any]) -> Optional[str]:
    ...
    def _parse_spell_from_definition(self, spell_data: Dict[str, Any]) -> Spell:
        ...
        if school is None:
            school = "Unknown"
        school = self.SPELL_SCHOOL_MAP.get(school, school)
        
        # Casting info
        casting_time = self._format_casting_time(definition.get("activation", {}))
        range_str = self._format_range(definition.get("range", {}))
        duration = self._format_duration(definition.get("duration", {}))
        
        # Components
        components_list = definition.get("components", [])
        components_desc = definition.get("componentsDescription", "")
        components = self._parse_spell_components(components_list, components_desc)
        
        # Description
        description = self._clean_html(definition.get("description", ""))
        ...
        # Area of effect
        area = self._format_area(definition.get("range", {}))
    ...
    def _get_ability_score(self, ability_id: int) -> int:
    ...
    def _parse_spellcasting_info(self, class_data: Dict[str, Any]) -> Optional[SpellcastingInfo]:
        ...
        ability_name = self.ABILITY_ID_TO_NAME.get(ability_id, "charisma")
        
        # Get ability score and calculate modifier
        ability_score = self._get_ability_score(ability_id)
        ability_modifier = self._calculate_ability_modifier(ability_score)
        
        # Calculate total level for proficiency bonus
        total_level = sum(cls.get("level", 1) for cls in self.data.get("classes", []))
        ...
        if class_name.lower() == "warlock":
            ...
            warlock_slots = {
                1: {3: 1}, 2: {3: 2}, 3: {3: 2}, 4: {3: 2}, 5: {3: 2}, 6: {3: 2},
    ...
    def _organize_spells_by_level(self, spells: List[Spell]) -> Dict[str, List[Spell]]:
    ...
    def _parse_dndbeyond_spelllist(self) -> SpellList:
        """Parse D&D Beyond JSON data and create SpellList object."""
        ...
        for class_data in classes:
            ...
            spell_info = self._parse_spellcasting_info(class_data)
            if spell_info:
        ...
        class_spells_data = self.data.get("classSpells", [])
        ...
        for class_spell_data in class_spells_data:
            ...
            for spell_data in spells_list:
                try:
                    spell = self._parse_spell_from_definition(spell_data)
            ...
            if class_spells:
                spells_by_class[class_name] = self._organize_spells_by_level(class_spells)
        ...
        if other_spells and spellcasting_info:
            ...
            other_spells_by_level = self._organize_spells_by_level(other_spells)
